...
def calculate_queueing_time(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["metadata.queueing_time"] = (df["metadata.started"] - df["metadata.submission_time"]).dt.total_seconds() / 60
...
def calculate_per_job_goodput(df: pd.DataFrame) -> pd.DataFrame:
    ...
    total_elapsed_time = (df["metadata.completed"] - df["metadata.submission_time"]).dt.total_seconds()

    # Calculate training time as difference between completed and started
    train_time = (df["metadata.completed"] - df["metadata.started"]).dt.total_seconds()
    ...
    df["metadata.per_job_goodput"] = np.where(
        ...
        0.0,
        np.where(total_elapsed_time > 0, train_time / total_elapsed_time, 0.0),
    )
...
def calculate_overall_goodput_with_failed_jobs(df: pd.DataFrame) -> float:
    ...
    valid_jobs = df_copy[
        df_copy["metadata.train_runtime"].notna() & (df_copy["metadata.train_runtime"] >= 0) & (elapsed_time >= 0)
    ].copy()
    ...
    total_train_runtime = (valid_jobs["metadata.completed"] - valid_jobs["metadata.started"]).dt.total_seconds().sum()
    total_elapsed_time = (df_copy["end_time"] - df_copy["metadata.submission_time"]).dt.total_seconds().sum()
...
def calculate_overall_goodput(df: pd.DataFrame) -> float:
    ...
    total_elapsed_time = (
        (valid_jobs["metadata.completed"] - valid_jobs["metadata.submission_time"]).dt.total_seconds().sum()
    )
...
def identify_invalid_elapsed_time_jobs(
    df: pd.DataFrame, output_csv: str = "invalid_elapsed_time_jobs.csv"
) -> pd.DataFrame:
    """
