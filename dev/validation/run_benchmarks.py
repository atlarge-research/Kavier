...
def run_benchmark(df: pd.DataFrame, name: str, filter_fn=None) -> Dict:
    """Run the engine over an (optionally filtered) slice and return percent-error stats, or None if empty."""
    if filter_fn:
...
def main():
    """Run all benchmark slices against the bundled validation CSV and print a MAPE summary."""
