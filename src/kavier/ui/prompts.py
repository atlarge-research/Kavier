"""Keyboard-driven interactive widgets built on Rich alone. Cancel (Ctrl-C / Esc) raises ``Abort``."""
...
def read_key() -> str:
    ...
    try:
        ...
        if not ch:  # EOF
            return "esc"
        if ch == b"\x1b":  # arrow suffix follows only if pending within 50ms
...
@dataclass
class Choice:
    """A selectable option: ``value`` is returned, ``label`` shown, ``hint`` dimmed."""
