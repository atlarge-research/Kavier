...
def spinner(message: str, accent: str = "cyan") -> Iterator[None]:
    """Context manager that shows a Rich progress spinner while its body runs."""
    with Progress(
...
def specs_panel(model: str, gpu: str, accent: str = "cyan") -> Panel:
    """Rich Panel showing LLM and GPU spec side-by-side."""
    llm = get_llm(model)
    g = get_gpu(gpu)
    m = _kv_table(
        [
...
def inference_result(r: dict[str, Any]) -> Panel:
    """Rich Panel with latency/throughput/cache results from a ``run_inference`` dict."""
    config = _kv_table(
        [
    ...
    return Panel(
        Columns([config, table], expand=True), title="[bold cyan]Inference[/]", border_style="cyan", padding=(1, 2)
    )


def training_result(r: dict[str, Any]) -> Panel:
    """Rich Panel with throughput/MFU/power results from a ``run_training`` dict."""
    config = _kv_table(
        [
            ("method", r["method"]),
    ...
    return Panel(
        Columns([config, _result_table("Training results", rows, "magenta")], expand=True),
        title="[bold magenta]Training[/]",
...
def energy_result(r: dict[str, Any]) -> Panel:
    """Rich Panel with Wh/kWh/$/Mtoken results from an ``energy_from_inference`` dict."""
    ...
    return Panel(
        _result_table("Energy & efficiency", rows, "green"),
...
def carbon_result(r: dict[str, Any]) -> Panel:
    ...
    return Panel(
        _result_table("Carbon emissions", rows, "yellow"),
        title="[bold yellow]Carbon[/]",
        border_style="yellow",
        padding=(1, 2),
    )
