"""
Run all training simulation benchmarks and compute MAPE.
This script validates the training simulator against real data.
"""
...
def run_benchmark(df: pd.DataFrame, name: str, filter_fn=None) -> Dict:
    """Run benchmark on filtered dataset."""
    if filter_fn:
...
def main():
    ...
    for gpu in sorted(df['gpu_model'].unique()):
        benchmarks.append(run_benchmark(df, f"{gpu}", lambda d, g=gpu: d['gpu_model'] == g))
        benchmarks.append(run_benchmark(df, f"{gpu} (1-GPU)", lambda d, g=gpu: (d['gpu_model'] == g) & (d['number_gpus'] == 1)))
        benchmarks.append(run_benchmark(df, f"{gpu} (2-GPU)", lambda d, g=gpu: (d['gpu_model'] == g) & (d['number_gpus'] == 2)))
    ...
    for b in benchmarks:
        if b:
            print(f"{b['name']:<40} {b['samples']:>8} {b['mape']:>7.1f}% {b['median_ape']:>7.1f}% {b['min_error']:>7.1f}% {b['max_error']:>7.1f}%")
