...
def _pick_model(accent: str, default: str = _DEFAULT_MODEL) -> str:
    ...
    return str(prompts.fuzzy_select("Model", choices, accent=accent,
...
def _pick_gpu(accent: str, default: str = _DEFAULT_GPU) -> str:
    ...
    return str(prompts.fuzzy_select("GPU", choices, accent=accent,
                                    default=default if default in names else names[0]))
...
def _inference_inputs(seed: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    num_requests = prompts.number_prompt("Number of requests:", default=s.get("num_requests", 128),
                                         minimum=1, accent=accent)
    input_tokens = prompts.number_prompt("Input tokens / request:", default=s.get("input_tokens", 512),
                                         minimum=1, accent=accent)
    output_tokens = prompts.number_prompt("Output tokens / request:", default=s.get("output_tokens", 128),
                                          minimum=1, accent=accent)
    kv_cache = prompts.confirm("Enable KV cache?", default=s.get("kv_cache", True), accent=accent)
    policy = prompts.menu("Prefix-cache policy", [
    ...
    ], accent=accent, default={"prefill": 0, "full": 1, "none": 2}.get(s.get("prefix_policy", "prefill"), 0))
    return {"model": model, "gpu": gpu, "num_requests": num_requests, "input_tokens": input_tokens,
            "output_tokens": output_tokens, "kv_cache": kv_cache, "prefix_policy": policy,
...
def _inference_followups(result: dict[str, Any], inputs: dict[str, Any]) -> None:
    while True:
        ...
        if action == "rerun":
            ...
            return
        if action == "energy":
            price = prompts.number_prompt("GPU $/hour (0 to skip cost):", default=2.5,
                                          minimum=0.0, accent="green", integer=False)
            ...
            intensity = prompts.number_prompt("Carbon intensity (gCO2/kWh):", default=400,
                                              minimum=1, accent="yellow")
            ...
            try:
                ...
                console.print(f"[green]  ✓ wrote[/] [cyan]{out}/tasks.parquet[/] [green]+[/] "
                              f"[cyan]{out}/fragments.parquet[/]")
...
def _training_inputs(seed: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    total_tokens = prompts.number_prompt("Total tokens to train (0 to skip runtime):",
                                         default=s.get("total_tokens", 10_000_000), minimum=0, accent=accent)
    return {"model": model, "gpu": gpu, "method": method, "batch_size": batch_size, "seq_len": seq_len,
            "num_gpus": num_gpus, "num_nodes": num_nodes, "total_tokens": total_tokens}
...
def main() -> None:
    if not sys.stdin.isatty():
        console.print("[yellow]The Kavier UI needs an interactive terminal. "
