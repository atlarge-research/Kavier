...
def test_one_row_per_node_including_idle_nodes() -> None:
    ...
    res = schedule([{"submit_s": 0, "gpus": 2, "duration_s": 10}], policy="fcfs", num_nodes=4, node_gpus=8)
...
def test_node_busy_seconds_sum_to_total_gpu_seconds() -> None:
    ...
    res = schedule(jobs, policy="backfill", num_nodes=2, node_gpus=8)
...
def test_node_energy_apportioned_and_sums_to_cluster_total() -> None:
    ...
    res = schedule([job], policy="fcfs", num_nodes=2, node_gpus=8)
...
def test_full_node_utilization_is_one() -> None:
    ...
    res = schedule([{"submit_s": 0, "gpus": 8, "duration_s": 10}], policy="fcfs", num_nodes=1, node_gpus=8)
